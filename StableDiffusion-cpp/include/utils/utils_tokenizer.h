...
public:
    ...
    int load(const std::string &path) {
        std::string vocab = path + "/models/utils/vocab.txt";
        if (is_ch_) {
            vocab = path + "/models/utils/vocab.txt";
        }
        ...
        std::string merge = path + "/models/utils/merges.txt";
    ...
    std::vector<int> encode(std::string text) {
        ...
        while (regex_search(start, end, match_results, pat)) {
...
////            中文+英文
    ...
    std::vector<std::pair<std::string, float>> parse_prompt_attention(const std::string &prompt) {
        ...
        while (regex_search(start, end, m, re_attention)) {
            std::string text = m[0];
